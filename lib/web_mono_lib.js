...
function set(target, key, value, receiver) { 
    ...
    const do_w_elem = (obj.do_w === false)? false : true;
    ...
    if (typeof obj.display == "function") {
        let ref_bool = Reflect.set(target, key, value, receiver);
        update_vars(key, obj.display(value), do_w_elem);
        return ref_bool;
    }
    let ref_bool = Reflect.set(target, key, value, receiver);
    update_vars(key, value, do_w_elem);
    return ref_bool;
}

let D = new Proxy({}, { set: set });

const update_vars = (k, v, do_w_elem=true)=>{
    for (const elem of document.body.getElementsByTagName("*")) {
        ...
        if (subStr == k && elem.innerText != set_str) elem.innerText = set_str;
        if (subStr.includes(":") && !subStr.includes("=>")) {
            let arg = subStr.split(":")[0];
            const index = parseInt(subStr.split(":")[1]);
            let format = false;

            if (arg.substring(0,2) == "F+") {
                arg = arg.substring(2);
                format = true;
            }
            ...
            let i_val = v[index];
            if (isNaN(parseFloat(i_val)) == false) {
                i_val = parseFloat(i_val);
            }
            // if (i_val === undefined) continue;
            if (format && typeof i_val == "number") { 
                // console.log(`Arg: ${arg}, Index: ${index}`);
                elem.innerText = elem.getAttribute("content").replace(`{${subStr}}`, format_num(i_val));
            }
            else {
                // console.warn(`Tried to format but type isn't 'number'\nValue: ${i_val}`);
                elem.innerText = elem.getAttribute("content").replace(`{${subStr}}`, i_val);
            }
        }
        if (subStr[0] == "F" && subStr[1] == "+") {
            if (subStr.slice(2) != k || typeof v != "number") continue;
            elem.innerText = format_num(v);
        }
        if (subStr.includes("=>")) {
